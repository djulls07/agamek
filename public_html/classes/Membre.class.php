...
class Membre {																
  ...
  //constructeur pour creer un membre deja existant dans bdd
  public function createMembre($pseudo, $path)
  {
    try {
      if (file_exists($path)) {
	...
	$bddUser = $tmp2['user'];
	$bddDest = $tmp2['dest'];
	$bddPass = $tmp2['password'];
      } else {
	echo 'Error construct: infos DB missing';
      }
      //ici on a la bdd
      $pdo_options[PDO::ATTR_ERRMODE] = PDO::ERRMODE_EXCEPTION;
      $bdd = new PDO($bddDest, $bddUser, 
		     $bddPass, $pdo_options);
      $req = "SELECT * FROM users WHERE _pseudo = \"".$pseudo."\"";
      $rep = $bdd->query($req);
      if (($d=$rep->fetch())) {
	//on a une reponse
	return new Membre($d, $path);
      } else {
	return NULL;
      }
    } catch (Exception $e) {
  ...
  /** Celui qui permet de creer objet m a partir inscription 
      et ou activation_compte **/
  ...
  public function majListeJeuxBdd($path) {
    try {
      if (file_exists($path)) {
	$tmp = file_get_contents($path);
	$tmp2 = json_decode($tmp,true);
	$tmp2['dest'] = 'mysql:host=' . $tmp2['host'] . ';dbname=' .
	  $tmp2["name"];
	$this->_bddUser = $tmp2['user'];
	$this->_bddDest = $tmp2['dest'];
	$this->_bddPass = $tmp2['password'];
	...
	echo 'Error construct: infos DB missing';
      ...
      //ici on a la bdd
      $pdo_options[PDO::ATTR_ERRMODE] = PDO::ERRMODE_EXCEPTION;
      $bdd = new PDO($this->_bddDest, $this->_bddUser, 
		     $this->_bddPass, $pdo_options);
      $req = 'SELECT * FROM infos_jeux WHERE id_joueur="'.$this->_id.'"';
      $rep = $bdd->query($req);
      while($d=$rep->fetch()) {
	switch($d['id_jeu']) {
	case 1: 
	  $this->_listeJeux['sc2'] =  new InfoJeuSC2($d['pseudo'], 
						     $d['region']);
	  break;
	case 2:
	  ...
	  break;
	case 3:
	  $this->_listeJeux['lol'] = new InfoJeuLoL($d['pseudo'], 
						    $d['region']);
	  break;
	default:
	  echo "ERROR: id = " . $d->id_jeu;
	  break;
	}
	$new_r = 'InfoJeu'.$new;				
      }
  ...
  /* Affiche un resume du profil user ( pseudo liste jeux joué, sans details. */
  public function afficheResume()
  {
    echo '<article title="resume_membre">';
    if ($this->isThisLogIn()) {
      $this->afficheElement("_pseudo");
      $this->afficheElement("_avatar");
      $this->afficheElement("_email");
      $this->afficheElement("_prenom");
      $this->afficheElement("_nom");
      $this->afficheElement("_signature");
      $this->afficheElement("_localisation");
    } else {
	$this->afficheElement("_pseudo");
	$this->afficheElement("_avatar");
	$this->afficheElement("_signature");
    ...
    echo '</article>';
    echo '<article title="resume_jeux">';
    echo '</article><br />';
  }

  public function afficheElement($nom)
  {
    if ($this->$nom != NULL && strlen($this->$nom) > 1) {
      $n_nom = substr($nom, 1);
      echo $n_nom . ' : ' . $this->$nom . '<br />';
    }
  }
  
  /* Affiche les infos de profil des users*/
  public function afficheProfil()
  {
    echo 
      '<section> <header> <h1> ' . $_SESSION['text']['profil_d'] . ' ' .
      $this->_pseudo . '</h1></header>';
    
    $this->majListeJeuxBdd("infos/infos_bdd");

    $this->afficheResume();

    echo '<br />';

    foreach($this->_listeJeux as $key => $val) {
      echo '<article class="profil_jeu" id="'.$key.'">';
      $val->afficheResume();
      echo '<br />';
      $val->afficheDonnees();
    ...
    if ($this->isThisLogIn()) {
      //afficher l'option edit profile
      echo '<a href="?page=edit_profile" title="edit profile">' .
	$_SESSION['text']['edit_profil'] . '</a>';
    }
    echo '</section>'; 
  }  
  ...
  /* Permet de savoir si le mec log (session) est celui dont on regarde le profil
   */
  public function isThisLogIn()
  {
    return (is_log() && 
	    unserialize($_SESSION['user'])->getId() == $this->_id);
  }
  ...
  public function loadConversations() {
    try {
      ...
      $bdd = new PDO($this->_bddDest, $this->_bddUser, 
		     $this->_bddPass,$pdo_options);
      //TODO: finiir ça et MESSAGES.PHP
      $req = "SELECT * FROM conversations WHERE id_user1 =\"" . 
	$this->_id ."\" OR id_user2=\"".$this->_id . "\" ORDER BY date DESC";
      $rep = $bdd->query($req);
      while(($d=$rep->fetch())) {
	/* Pour chaque conversations ( 10 1er) on charge les messages*/
	//TODO : recup pseudo user1 et 2 pour coherence !
	$this->_id == $d["id_user1"] ? $id_d=$d["id_user2"]:$id_d=$d['id_user1'];
	array_push($this->_conversations, array("user1" => $d["user1"], "user2" => $d["user2"], "titre" => $d['titre'], "contenu" => $d["contenu"]));
      }
    } catch(Exception $e) {
      die("Error : " . $e->getMessage());
    }
  ...
  public function getJsonFromConversations() {
	return json_encode($this->_conversations);
  }
