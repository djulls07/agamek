...
class InfoJeuSC2 extends InfoJeu {
  ...
  /* method qui fait les diff appels a sc2 api*/
  public function recup_info_api($b) {   
    $ch = curl_init();
    ...
    $data = array('name' => $this->_pseudo, 'bracket' => $b, 
  ...
  /* On charge les differentes infos pour chaque bracket dans donnees */	
  public function recup_infos_api() {
    $i = 0;
    $this->_donnees = array();
    $this->_donnees["1v1"] = json_decode(self::recup_info_api("1v1"), true);
    for ($i=2; $i<5; $i+=2) {
      $this->_donnees[$i . 'v' . $i .'-Team'] = json_decode(self::recup_info_api($i . 'v' . $i .'t'), true);
      $this->_donnees[$i . 'v' . $i .'-Random'] = json_decode(self::recup_info_api($i . 'v' . $i .'r'), true);
    }    
  }
  ...
  public function ecrire_bdd($id_user) {
	...
	$bdd = new PDO("mysql:host=".$tab['host'].";dbname=".$tab['name'], 
		       $tab['user'], $tab['password'], $pdo_options);
	$req = $bdd->prepare("INSERT INTO infos_jeux(id_joueur, id_jeu, pseudo, region) VALUES(".
			     $id_user.",?,?,?)");
  ...
  public function afficheDonnees()
  {
    ...
    foreach($this->_donnees as $key => $val) {
      if (!empty($val)) {
	$str = affiche_sc2_stats($val);
	echo $_SESSION["text"]["Gstats"] . " " . $key . " : <br />" . $str;
      }
    }
  }
...
class InfoJeuDota2 extends InfoJeu {
  ...
  public function recup_infos_api() 
  {
    ...
    $this->_donnees = '<a href="http://dotabuff.com/players/' . $this->_dotaId . '">'.
      $_SESSION['text']['profil'] . ' Dotabuff </a>';
  }
...
class InfoJeuLoL extends InfoJeu {
  ...
  private $_id_jeu = 3;
  private $_summId;
  private $_api_key = "fe8ad5ae-034e-43eb-944f-83ac6cccc1a1";
  private $_tableauTriAffiche = array('RankedSolo5x5','Unranked');
  ...
  public function recup_infos_api()
  {
    ...
    $this->_donnees = array();
    $ua = "AGAMEK";
    $target1 = "https://prod.api.pvp.net/" .
      "api/lol/".$this->_region."/v1.2/stats/by-summoner/".
      $this->_summId."/ranked?".
      "api_key=fe8ad5ae-034e-43eb-944f-83ac6cccc1a1";
    
    $target2 = "https://prod.api.pvp.net/" .
      "api/lol/".$this->_region."/v1.2/stats/by-summoner/".
      $this->_summId."/summary?".
      "api_key=fe8ad5ae-034e-43eb-944f-83ac6cccc1a1";
    
    $ch = curl_init($target1);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    $out = curl_exec($ch);

    /*check http code si 404 ignorer*/
    $httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    if ($httpcode == 404) {
      $this->_donnees[0] = $_SESSION["text"]["no_ranked_g"];
    } else {
      $this->_donnees[0] = array();
      $this->_donnees[0] = json_decode($out, true);
    }
    curl_close($ch);
    ...
    $httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
    if ($httpcode == 404) {
      $this->_donnees[1] = $_SESSION["text"]["no_public_g"];
    } else {
      $this->_donnees[1] = array();
      $this->_donnees[1] = json_decode($out, true);
    }
    curl_close($ch);
  }

  public function recup_id()
  {
    $ua = "Deathgame site";
    $ch = curl_init("https://prod.api.pvp.net/api/lol/".
		    $this->_region."/v1.3/summoner/".
		    "by-name/".$this->_pseudo."?api_key=".
		    "fe8ad5ae-034e-43eb-944f-83ac6cccc1a1");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    $out = curl_exec($ch);
    curl_close($ch);
    
    //API RIOT renvoi un obj JSON.
    $tab = json_decode($out, true);
    if ($tab == NULL) $this->_summId = NULL;
    else $this->_summId = $tab[$this->_pseudo]['id'];
  }
  ...
  public function afficheDonnees()
  {
    ...
    echo '<section id="lol_stats_container">';
    if (is_array($this->_donnees[0])) {
      if(in_array($v['playerStatSummaryType'], $this->_tableauTriAffiche)) {
	echo '<br />';
	echo '<article class="lol_stat_container">';
	affiche_array($v, 0);
	echo '</article>';
      }
    } else {
      echo $this->_donnees[1];
    }

    echo '<br />';
    
    if (is_array($this->_donnees[1])) {
      foreach($this->_donnees[1]["playerStatSummaries"] as $k => $v) {
	//on affiche que les types de stats qu'on veut !! 
	if(in_array($v['playerStatSummaryType'], $this->_tableauTriAffiche)) {
	  echo '<br />';
	  echo '<article class="lol_stat_container">';
	  affiche_array($v, 0);
