...
/* affiche un array de façon recursive, avec des esp
 * pour indenter. Si un le premier elem de a nest pas dans array on arrete.
 */

function affiche_array($a, $esp)
{
  foreach($a as $k => $v) {
    if (is_array($v)) {
      $esp += 4;
      affiche_array($v, $esp, $array);
    } else {
      for ($i=0;$i<$esp;$i++)
	echo '&nbsp';
      echo $k . ' : ' .$v . '<br />';
...
function affiche_sc2_stats($tab)
{
  foreach($tab as $k => $v) {
    return "League : " . $v["league"] . '<br />'.
      "Wins : " . $v["wins"] . "<br />".
      "Losses : " . $v["losses"] . "<br />".
      "Win Ratio : " . $v["win_ratio"] . '<br />';
  }
}
?>